...
This guide shows how to build a Retrieval Augmented Generation (RAG) application with [Deep Agents](/oss/python/deepagents). You index [docs.langchain.com](https://docs.langchain.com), retrieve relevant chunks at query time, offload them to the agent [filesystem](/oss/python/deepagents/backends), and delegate analysis to [subagents](/oss/python/deepagents/subagents) so the orchestrator context stays clean.

For a simpler RAG agent without subagents or filesystem offloading, see [Build a RAG agent with LangChain](/oss/python/langchain/rag).
...
The tool uses [`Command`](https://reference.langchain.com/python/langgraph/types/Command) to update the `files` state key. Deep Agents reads those paths through built-in filesystem tools such as `read_file` and `grep`.
...
For more on this topic, see the [security section](/oss/python/langchain/rag#security-considerations) of the LangChain RAG tutorial and research on [prompt injection](https://simonwillison.net/series/prompt-injection/).

## Next steps

- Compare this pattern with [Build a RAG agent with LangChain](/oss/python/langchain/rag) for a single-agent retrieval loop
